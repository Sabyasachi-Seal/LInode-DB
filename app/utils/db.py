...
from app.constants.enums import BackupSchedule
...
def validate_backup_schedule_inputs(hour_of_day: int, day_of_week: int, day_of_month: int, frequency: BackupSchedule):
    errors = []
    ...
    if frequency == BackupSchedule.daily.value:
        if day_of_week is not None or day_of_month is not None:
            errors.append("No day_of_week or day_of_month should be set for daily backups.")
        ...
        if day_of_week is None:
            ...
            errors.append("day_of_week must be an integer between 0 (Sunday) and 6 (Saturday).")
...
def convert_schedule_to_cron(hour_of_day: int, day_of_week: int, day_of_month: int, frequency: BackupSchedule) -> str:
    """
